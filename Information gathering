...
- Transfer process:  
  ...
  4. An employee with the **approver** role reviews the transfer details and, if verified, *approves* the transfer. This must be performed from a jump host.  
...
- OSINT of TheReserve’s corporate website, which is exposed on the external network of TheReserve. Note: this means that **all OSINT activities should be limited to the provided network subnet** and no external-internet OSINTing is required.  
- Phishing of any of TheReserve employees.  
...
![image1](https://github.com/Nourhansherf/DEPI-graduation-project/blob/master/images/image%201.png)
...
![image2](images/image%202.png)  
![image3](images/image%203.png)
...
Because we have a web server let’s try connecting on it:

1. open the `/etc/hosts` file to add the IP address of the web server  
   ![image4](images/image%204.png)
2. adding the IP address and the domain  
   ![image5](images/image%205.png) 
3. try to ping it to check the connection  
